...
{
	if (!_vrWindow || !_previewWindow || !_copyEyeToScreenMaterial)
	{
		return;
	}
...
void CameraManager::UpdateCameraAmbientColor(float delta)
{
	...
	{
		_cameraTargetAmbientColorIsWaitingForLastFrame = false;
		if (_cameraTargetAmbientColorCompletedCallback)
		{
			_cameraTargetAmbientColorCompletedCallback();
		}
	}

	if (_cameraTargetAmbientColorChangeRate == RN::Color::Black())
	{
		return;
	}
	if (delta > 0.2f)
...
void CameraManager::MovePancakeCamera(float delta)
{
	if (!_vrWindow && _headCamera && _headCamera->GetChildren()->GetCount() == 0)
	{
		...
		if (manager->IsControlToggling(RNCSTR("SHIFT")))
		{
			translation *= 4.0f;
		}
